import type { SocialAccount, SocialiteUi } from '@/types/socialite-ui';
...
export default function LinkedAccount({ socialAccount, socialiteUi }: LinkedAccountProps) {
      ...
      <div className="flex px-6 py-4 gap-4 border rounded-xl">
          <div className="grid w-full md:grid-cols-2 items-center gap-6">
              <div className="flex flex-row-reverse md:flex-row justify-between md:justify-start md:items-center gap-3">
                  <SocialiteProviderIcon provider={socialAccount.provider.id} className="h-6 w-6 md:h-8 md:w-8"/>
              ...
              <div className="flex flex-col md:flex-row items-center justify-between md:justify-end gap-3 md:gap-2">
                  ...
                  {!socialiteUi.hasPassword && (
                    <Tooltip delayDuration={0}>
                        <TooltipTrigger className="flex w-full md:w-auto">
                            <Button variant="destructive" disabled={!socialiteUi.hasPassword} className="w-full">
                                Unlink
                            </Button>
                        </TooltipTrigger>
                        <TooltipContent>
                  ...
                  {socialAccount && socialiteUi.hasPassword && (
                    <Dialog>
                        <DialogTrigger asChild>
                            <Button variant="destructive" disabled={!socialiteUi.hasPassword} className="w-full md:w-auto">
                                Unlink
                            </Button>
                        </DialogTrigger>
                        <DialogContent>
